        ...
        <frm:form method="put" modelAttribute="cartItemsQuantities" action="${pageContext.servletContext.contextPath}/cart">
          <table class="table table-hover table-bordered text-center">
            ...
            <c:forEach var="i" begin="0" end="${cart.items.size() - 1}">
              <tr id="row${cart.items[i].car.id}">
                <td class="align-middle">
                  <a href="<c:url value="/productDetails/${cart.items[i].car.id}"/>">${cart.items[i].car.brand}</a>
                </td>
                <td class="align-middle">
                  <a href="<c:url value="/productDetails/${cart.items[i].car.id}"/>">${cart.items[i].car.model}</a>
                </td>
                <td class="align-middle">
                  <ul>
                    <c:forEach var="color" items="${cart.items[i].car.colors}">
                      <li>${color.code}</li>
                    </c:forEach>
                  </ul>
                </td>
                <td class="align-middle">${cart.items[i].car.displaySizeInches}"</td>
                <td class="align-middle">$ ${cart.items[i].car.price}</td>
                <td class="align-middle">
                  <frm:input path="items[${i}].quantity"/>
                  <frm:hidden path="items[${i}].carId"/>
                ...
                <td class="align-middle">
                  <button form="deleteCartItem" class="btn btn-outline-dark border-dark"
                          formaction="<c:url value="/cart?carId=${cart.items[i].car.id}"/>">Delete</button>
                </td>
              </tr>
            </c:forEach>
          </table>
